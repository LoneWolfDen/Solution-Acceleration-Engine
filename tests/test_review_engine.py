...
--------
Schema:
  reviews table present after init_database(); SCHEMA_VERSION == 3;
...
async def test_schema_version_is_3(db) -> None:
    """SCHEMA_VERSION constant and stored DB version must both be 3."""
...
async def test_migration_v2_to_v3_creates_reviews_table(tmp_path) -> None:
    ...
    try:
        ...
        assert row[0] == 3, f"schema_version not updated to 3, got {row[0]}"
    finally:
        await conn2.close()
