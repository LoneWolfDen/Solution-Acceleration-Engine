...
Routes to one of three views based on AppState.active_view:

    "welcome"  →  WelcomeView  (nothing selected)
    ...
    return rx.box(
        rx.vstack(
            ...
            rx.cond(
                ...
                rx.text(
                    "Select a version to view its artifacts and reviews.",
                    size="2",
                    color_scheme="gray",
                    text_align="center",
                ),
...
def content_pane() -> rx.Component:
    """
    Master content area.  Routes to the correct pane based on AppState.active_view.
    Wraps a loading overlay around all views.
    """
    return rx.box(
        rx.cond(
            AppState.is_loading,
