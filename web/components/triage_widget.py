...
Renders the artifact triage table after an artifact is saved:
  - Table: title | tags | is_active toggle.
  - Newly saved artifact highlighted.
  - [Create Version] button gated on at least one active artifact.
"""
...
def _triage_row(artifact: dict) -> rx.Component:
    is_new = (
        AppState.last_saved_artifact.get("artifact_id", "") == artifact["artifact_id"]
    )
    return rx.hstack(
        # Title
        rx.text(
...
def triage_widget() -> rx.Component:
    """Full triage widget with table and Create Version button."""
    return rx.vstack(
        # Header row
        rx.hstack(
        ...
        rx.cond(
            ...
            rx.center(
                rx.text(
                    "No artifacts yet. Save one to begin triaging.",
                    size="2",
                    color_scheme="gray",
                ),
                padding="1.5rem",
        ...
        rx.hstack(
            rx.text(
                rx.fragment(
                    AppState.triage_active_artifact_ids.length(),
                    " artifact(s) selected",
                ),
                size="2",
                color_scheme="gray",
