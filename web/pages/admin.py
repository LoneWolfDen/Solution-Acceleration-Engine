...
Sections:
  1. System Health — provider status pills from GET /api/admin/health.
  2. LLM Configuration — API key fields + Ollama URL.
  3. Gate Thresholds — numeric inputs per dimension.
  4. Project Management — table with cascade-delete buttons.
"""
...
def _provider_pill(name: str, status: str) -> rx.Component:
    is_ok = status == "configured" or status == "set"
    return rx.hstack(
        rx.icon(
        ...
        rx.badge(
            status,
            color_scheme=rx.cond(is_ok, "green", "gray"),
            variant="soft",
            size="1",
        ),
        spacing="2",
...
def _health_section() -> rx.Component:
    ...
    return _section_card(
        "System Health",
        rx.hstack(
            _provider_pill("Groq", providers.get("groq", "not_set")),
            _provider_pill("OpenRouter", providers.get("openrouter", "not_set")),
            _provider_pill("Gemini", providers.get("gemini", "not_set")),
            _provider_pill("Ollama", providers.get("ollama", "not_set")),
            spacing="3",
            flex_wrap="wrap",
        ),
        rx.hstack(
            ...
            rx.text(
                rx.cond(last_run, last_run, "Never"),
                size="2",
...
def _api_key_field(provider: str) -> rx.Component:
    """API key input row: shows masked status or empty field."""
    config = AppState.admin_config
    providers_cfg = config.get("providers", {})
    key_status = providers_cfg.get(provider, "not_set")
    is_set = key_status == "set"
    placeholder = rx.cond(is_set, "••••••••  (currently set)", "Paste API key here…")

    key_var = rx.State.create_ref(f"_key_{provider}")
    return rx.hstack(
        ...
        rx.badge(
            key_status,
            color_scheme=rx.cond(is_set, "green", "gray"),
            variant="soft",
            size="1",
        ),
        spacing="3",
...
def _threshold_row(label: str, key: str, default: float) -> rx.Component:
    config = AppState.admin_config
    thresholds = config.get("thresholds", {})
    current_val = thresholds.get(key, default)
...
def _thresholds_section() -> rx.Component:
    return _section_card(
        "Gate Thresholds",
        _threshold_row("Risk threshold", "risk", 0.75),
        _threshold_row("Constraint threshold", "constraint", 0.70),
        _threshold_row("Dependency threshold", "dependency", 0.80),
    )
...
def _project_row(project: dict) -> rx.Component:
    return rx.hstack(
        rx.vstack(
            ...
            rx.text(
                rx.fragment(project.get("version_count", 0), " versions  ·  ", project.get("review_count", 0), " reviews"),
                size="1",
                color_scheme="gray",
            ),
            spacing="1",
            ...
            rx.alert_dialog.content(
                ...
                rx.alert_dialog.description(
                    rx.fragment(
                        ...
                        project["name"],
                        "' and all its versions, artifacts, and reviews. This cannot be undone.",
                    ),
                    size="2",
                    color_scheme="gray",
                ),
